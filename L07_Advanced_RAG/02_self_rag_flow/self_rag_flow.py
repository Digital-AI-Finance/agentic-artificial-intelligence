...
for x, y, label, color, shape in nodes:
    if shape == "diamond":
        ...
        diamond = plt.Polygon(
            [(x, y + 0.5), (x + 0.6, y), (x, y - 0.5), (x - 0.6, y)],
            facecolor="white",
            edgecolor=color,
            linewidth=2,
        )
...
for x1, y1, x2, y2, label in arrows:
    ax.annotate(
        "", xy=(x2, y2), xytext=(x1, y1), arrowprops=dict(arrowstyle="->", color=MLGRAY, lw=1.5)
    )
    if label:
...
ax.annotate(
    ...
    xy=(3.5, 2.9),
    xytext=(7.5, 2.9),
    arrowprops=dict(arrowstyle="->", color=MLRED, lw=1.5, connectionstyle="arc3,rad=-0.3"),
)
...
plt.tight_layout()
plt.savefig(Path(__file__).parent / "self_rag_flow.pdf", dpi=300, bbox_inches="tight", facecolor="white")
plt.close()
print(f"Chart saved to {Path(__file__).parent / 'self_rag_flow.pdf'}")
