...
ax.text(5, 5.7, 'CRAG: Corrective RAG Architecture', fontsize=24,
...
# Main arrows
ax.annotate('', xy=(2.3, 3.5), xytext=(1.9, 3.5),
...
for x, y, label, color, (tx, ty), action in outcomes:
    ax.annotate('', xy=(tx, ty), xytext=(5.7, y),
                arrowprops=dict(arrowstyle='->', color=color, lw=1.5))
    ax.text(tx, ty, action, fontsize=10, ha='left', color=color)

# Merge to generator
ax.annotate('', xy=(7.3, 3.5), xytext=(7.0, 4.3),
            arrowprops=dict(arrowstyle='->', color=MLGRAY, lw=1.5))
ax.annotate('', xy=(7.3, 3.5), xytext=(7.0, 3.5),
            arrowprops=dict(arrowstyle='->', color=MLGRAY, lw=1.5))
ax.annotate('', xy=(7.3, 3.5), xytext=(7.0, 2.7),
            arrowprops=dict(arrowstyle='->', color=MLGRAY, lw=1.5))

# Web search box
web_box = FancyBboxPatch((5.8, 1.6), 1.4, 0.6,
                         boxstyle="round,pad=0.05", facecolor='#FFF0E0',
                         edgecolor=MLRED, linewidth=1.5, linestyle='--')
...
ax.text(5.0, 0.6, 'Correct > 0.7 | Ambiguous 0.3-0.7 | Incorrect < 0.3', fontsize=10, ha='center', color=MLGRAY)
