...
ax.text(5, 5.7, 'Agent Role Specialization', fontsize=15,
        fontweight='bold', ha='center', color=MLPURPLE)

# Role boxes
roles = [
    ('Orchestrator', 1.5, 4.0, MLPURPLE, [
        'Task routing',
        'Workflow control',
        'Result aggregation'
    ]),
    ('Planner', 4.0, 4.0, MLBLUE, [
        'Task decomposition',
        'Strategy selection',
        'Resource allocation'
    ]),
    ('Executor', 6.5, 4.0, MLGREEN, [
        'Tool invocation',
        'Code execution',
        'API calls'
    ]),
    ('Critic', 2.75, 1.5, MLORANGE, [
        'Output validation',
        'Quality checks',
        'Error detection'
    ]),
    ('Specialist', 5.25, 1.5, MLRED, [
        'Domain expertise',
        'Knowledge retrieval',
        'Custom skills'
    ]),
...
for name, x, y, color, skills in roles:
    # Main box
    box = FancyBboxPatch((x - 1.0, y - 1.2), 2.0, 2.0,
                         boxstyle="round,pad=0.05", facecolor='white',
                         edgecolor=color, linewidth=2)
    ax.add_patch(box)

    # Header
    header = FancyBboxPatch((x - 1.0, y + 0.4), 2.0, 0.45,
