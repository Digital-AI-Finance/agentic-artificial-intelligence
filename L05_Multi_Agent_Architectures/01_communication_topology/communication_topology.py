...
# Arrows
ax2.annotate(
    "", xy=(-0.7, 0.3), xytext=(-0.2, 0.9), arrowprops=dict(arrowstyle="->", color=MLGRAY, lw=1.5)
)
ax2.annotate(
    "", xy=(0.7, 0.3), xytext=(0.2, 0.9), arrowprops=dict(arrowstyle="->", color=MLGRAY, lw=1.5)
)
for x1, x2 in [(-1.3, -0.9), (-0.5, -0.9), (0.5, 0.9), (1.3, 0.9)]:
    ax2.annotate(
        "", xy=(x1, -0.9), xytext=(x2, -0.3), arrowprops=dict(arrowstyle="->", color=MLGRAY, lw=1)
    )
...
for i, (x1, y1) in enumerate(mesh_positions):
    for j, (x2, y2) in enumerate(mesh_positions):
        if i < j:
            ax3.plot([x1 * 0.75, x2 * 0.75], [y1 * 0.75, y2 * 0.75], color=MLGRAY, alpha=0.3, lw=1)

plt.tight_layout()
plt.savefig(
    Path(__file__).parent / "communication_topology.pdf",
    dpi=300,
    bbox_inches="tight",
    facecolor="white",
    edgecolor="none",
...
plt.close()
print(f"Chart saved to {Path(__file__).parent / 'communication_topology.pdf'}")
