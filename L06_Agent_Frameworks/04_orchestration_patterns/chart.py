...
ax.text(5, 5.7, 'Orchestration Patterns', fontsize=24,
...
for i, (x, y) in enumerate(seq_nodes):
    ...
    if i < len(seq_nodes) - 1:
        ax.annotate('', xy=(x + 0.7, y), xytext=(x + 0.3, y),
...
for i, y_off in enumerate([-0.6, 0, 0.6]):
    ...
    ax.annotate('', xy=(7.25, 4.0 + y_off), xytext=(6.3, 4.0),
                arrowprops=dict(arrowstyle='->', color=MLBLUE, lw=1))
    ax.annotate('', xy=(8.7, 4.0), xytext=(7.75, 4.0 + y_off),
                arrowprops=dict(arrowstyle='->', color=MLBLUE, lw=1))
...
for i, (x, y) in enumerate([(3.0, 2.2), (3.0, 1.5), (3.0, 0.8)]):
    ...
    ax.annotate('', xy=(x - 0.25, y), xytext=(1.85, 1.5),
...
for i, x in enumerate([6.3, 7.5, 8.7]):
    ...
    ax.annotate('', xy=(x, 1.05), xytext=(7.5, 1.45),
