...
ax.text(5, 5.7, 'Model Context Protocol (MCP) Architecture', fontsize=15,
        fontweight='bold', ha='center', color=MLPURPLE)

# Host Application (left)
host_box = FancyBboxPatch((0.3, 2.0), 2.8, 2.5,
                          boxstyle="round,pad=0.05", facecolor='#E8E8FF',
                          edgecolor=MLPURPLE, linewidth=2)
ax.add_patch(host_box)
ax.text(1.7, 4.2, 'Host Application', fontsize=11, fontweight='bold',
        ha='center', color=MLPURPLE)

# LLM inside host
llm_box = FancyBboxPatch((0.5, 3.0), 2.4, 0.8,
...
client_box = FancyBboxPatch((0.5, 2.2), 2.4, 0.6,
                            boxstyle="round,pad=0.02", facecolor='white',
...
ax.text(5, 3.5, 'JSON-RPC 2.0', fontsize=10, ha='center', color=MLGRAY,
        fontweight='bold', rotation=0)
ax.text(5, 3.1, 'stdio / HTTP', fontsize=9, ha='center', color=MLGRAY)

# Arrows
ax.annotate('', xy=(3.8, 3.3), xytext=(3.1, 3.3),
            arrowprops=dict(arrowstyle='->', color=MLGRAY, lw=2))
ax.annotate('', xy=(3.1, 2.7), xytext=(3.8, 2.7),
            arrowprops=dict(arrowstyle='->', color=MLGRAY, lw=2))

ax.annotate('', xy=(6.9, 3.3), xytext=(6.2, 3.3),
            arrowprops=dict(arrowstyle='->', color=MLGRAY, lw=2))
ax.annotate('', xy=(6.2, 2.7), xytext=(6.9, 2.7),
            arrowprops=dict(arrowstyle='->', color=MLGRAY, lw=2))

# MCP Server (right)
server_box = FancyBboxPatch((6.9, 1.5), 2.8, 3.5,
                            boxstyle="round,pad=0.05", facecolor='#E8FFE8',
                            edgecolor=MLGREEN, linewidth=2)
ax.add_patch(server_box)
ax.text(8.3, 4.7, 'MCP Server', fontsize=11, fontweight='bold',
...
for name, y, color in components:
    box = FancyBboxPatch((7.1, y), 2.4, 0.6,
                         boxstyle="round,pad=0.02", facecolor='white',
