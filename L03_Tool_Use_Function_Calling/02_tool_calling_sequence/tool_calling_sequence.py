...
ax.text(
    5, 5.7, "Function Calling Sequence", fontsize=15, fontweight="bold", ha="center", color=MLPURPLE
)
...
for x1, x2, y, label, color in messages:
    ax.annotate(
        "", xy=(x2, y), xytext=(x1, y), arrowprops=dict(arrowstyle="->", color=color, lw=1.5)
    )
    mid_x = (x1 + x2) / 2
...
plt.tight_layout()
plt.savefig(
    Path(__file__).parent / "tool_calling_sequence.pdf",
    dpi=300,
    bbox_inches="tight",
    facecolor="white",
    edgecolor="none",
...
plt.close()
print(f"Chart saved to {Path(__file__).parent / 'tool_calling_sequence.pdf'}")
