...
ax.text(5, 5.7, 'Function Calling API Comparison', fontsize=24,
        fontweight='bold', ha='center', color=MLPURPLE)

# Provider boxes
providers = [
    ('OpenAI', 1.5, 3.5, MLPURPLE, ['tools parameter', 'JSON Schema', 'Parallel calls', 'Strict mode']),
    ('Anthropic', 5.0, 3.5, MLBLUE, ['tool_use blocks', 'Input schema', 'Sequential', 'Prefill support']),
    ('Google', 8.5, 3.5, MLORANGE, ['function_calling', 'OpenAPI spec', 'Grounding', 'Code execution']),
]

for name, x, y, color, features in providers:
    # Main box
    box = FancyBboxPatch((x - 1.3, y - 2.0), 2.6, 3.2,
                         boxstyle="round,pad=0.05", facecolor='white',
                         edgecolor=color, linewidth=3)
    ax.add_patch(box)

    # Header
    header = FancyBboxPatch((x - 1.3, y + 0.8), 2.6, 0.45,
...
ax.annotate('', xy=(8.0, 1.0), xytext=(2.0, 1.0),
            arrowprops=dict(arrowstyle='<->', color=MLGREEN, lw=2))
ax.text(5, 0.6, 'All support: JSON schemas, streaming, error handling',
        fontsize=14, ha='center', color=MLGREEN, fontweight='bold')
