...
\begin{itemize}
\item \textbf{Remember}: Define function calling, MCP (Model Context Protocol), tool augmentation
...
\begin{itemize}
\item JSON-RPC 2.0 (structured messaging) over stdio or HTTP
...
\textbf{Tool Definition (JSON Schema = typed structure)}
\begin{verbatim}
...
\begin{frame}[t]{Tool Selection by Domain}
\begin{center}
\includegraphics[width=0.60\textwidth]{03_tool_selection/tool_selection.pdf}
\end{center}
\bottomnote{Tool selection accuracy varies by domain -- GPT-4 leads across all categories.}
\end{frame}

\begin{frame}[t]{API-Bank and ToolLLM}
\textbf{API-Bank (Li et al., 2023, arXiv:2304.08244)}
\begin{itemize}
\item Benchmark with 73 APIs across 3 levels of complexity
\item Tests tool selection, parameter extraction, composition
\item GPT-4 achieves 80\%+ accuracy on single-tool tasks
\end{itemize}
\vspace{0.3cm}
...
\textbf{Risks}
\begin{itemize}
\item \textbf{Prompt injection}: Malicious input that hijacks LLM behavior
...
\begin{frame}[t]{API Comparison}
\begin{center}
\includegraphics[width=0.65\textwidth]{04_api_comparison/api_comparison.pdf}
\end{center}
\bottomnote{All major providers support function calling with slightly different APIs.}
\end{frame}
