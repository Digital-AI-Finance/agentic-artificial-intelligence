...
# LLM extraction
ax.annotate(
    "", xy=(4.5, 4.2), xytext=(3.5, 4.2), arrowprops=dict(arrowstyle="->", color=MLPURPLE, lw=2)
)
...
# Graph output
ax.annotate(
    "", xy=(8.0, 3.8), xytext=(7.0, 4.2), arrowprops=dict(arrowstyle="->", color=MLGREEN, lw=2)
)
ax.annotate(
    "", xy=(8.0, 3.2), xytext=(7.0, 3.0), arrowprops=dict(arrowstyle="->", color=MLGREEN, lw=2)
)
...
plt.tight_layout()
plt.savefig(Path(__file__).parent / "entity_extraction.pdf", dpi=300, bbox_inches="tight", facecolor="white")
plt.close()
print(f"Chart saved to {Path(__file__).parent / 'entity_extraction.pdf'}")
