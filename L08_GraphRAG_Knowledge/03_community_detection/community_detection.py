...
for i in range(len(c1_nodes)):
    for j in range(i + 1, len(c1_nodes)):
        if np.random.random() > 0.3:
            ax.plot(
                [c1_nodes[i][0], c1_nodes[j][0]],
                [c1_nodes[i][1], c1_nodes[j][1]],
                "-",
                color=MLPURPLE,
                alpha=0.3,
                linewidth=1,
...
for i in range(len(c2_nodes)):
    for j in range(i + 1, len(c2_nodes)):
        if np.random.random() > 0.3:
            ax.plot(
                [c2_nodes[i][0], c2_nodes[j][0]],
                [c2_nodes[i][1], c2_nodes[j][1]],
                "-",
                color=MLBLUE,
                alpha=0.3,
                linewidth=1,
...
for i in range(len(c3_nodes)):
    for j in range(i + 1, len(c3_nodes)):
        if np.random.random() > 0.3:
            ax.plot(
                [c3_nodes[i][0], c3_nodes[j][0]],
                [c3_nodes[i][1], c3_nodes[j][1]],
                "-",
                color=MLORANGE,
                alpha=0.3,
                linewidth=1,
...
plt.tight_layout()
plt.savefig(Path(__file__).parent / "community_detection.pdf", dpi=300, bbox_inches="tight", facecolor="white")
plt.close()
print(f"Chart saved to {Path(__file__).parent / 'community_detection.pdf'}")
