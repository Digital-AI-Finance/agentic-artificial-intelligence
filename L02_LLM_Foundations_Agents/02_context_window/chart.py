...
for i, (model, size) in enumerate(zip(models, context_sizes)):
    ax.annotate(f'{size}K', xy=(i + 0.3, 102), fontsize=9, ha='center',
                color=MLGRAY, fontweight='bold')

ax.set_ylabel('Capability Score (%)', fontsize=13)
ax.set_xlabel('Model / Context Window', fontsize=13)
ax.set_title('Context Window Impact on Agent Capabilities', fontsize=15, fontweight='bold', color=MLPURPLE)
...
ax.annotate('Larger context enables\ncomplex agent workflows',
            xy=(4.5, 45), fontsize=10, ha='center',
