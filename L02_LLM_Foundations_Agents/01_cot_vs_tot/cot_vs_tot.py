...
for i, (x, y, label, color) in enumerate(cot_nodes):
    ...
    ax1.add_patch(box)
    ax1.text(x, y, label, fontsize=11, ha="center", va="center", fontweight="bold", color=color)

    # Draw arrow to next node
    if i < len(cot_nodes) - 1:
        ax1.annotate(
            "",
            xy=(2, cot_nodes[i + 1][1] + 0.35),
            xytext=(2, y - 0.35),
            arrowprops=dict(arrowstyle="->", color=MLGRAY, lw=2),
        )

# Add description
ax1.text(2, 0.4, "Linear reasoning chain", fontsize=10, ha="center", color=MLGRAY, style="italic")
...
ax2.text(
    3, 5.25, "Question", fontsize=10, ha="center", va="center", fontweight="bold", color=MLPURPLE
)
...
for x in level1_x:
    ...
    # Arrow from question
    ax2.annotate(
        "", xy=(x, 4.3), xytext=(3, 5.0), arrowprops=dict(arrowstyle="->", color=MLGRAY, lw=1.5)
    )
...
ax2.annotate(
    "", xy=(1.0, 3.0), xytext=(1.0, 3.8), arrowprops=dict(arrowstyle="->", color=MLGRAY, lw=1.5)
)
...
ax2.annotate(
    "", xy=(5.0, 3.0), xytext=(5.0, 3.8), arrowprops=dict(arrowstyle="->", color=MLGRAY, lw=1.5)
)
...
ax2.text(
    5.0, 1.45, "Answer", fontsize=10, ha="center", va="center", fontweight="bold", color=MLGREEN
)
ax2.annotate(
    "", xy=(5.0, 1.7), xytext=(5.0, 2.5), arrowprops=dict(arrowstyle="->", color=MLGREEN, lw=2)
)

# Add description
ax2.text(3, 0.4, "Explore multiple paths", fontsize=10, ha="center", color=MLGRAY, style="italic")
ax2.text(3, 0.1, "Evaluate and prune", fontsize=9, ha="center", color=MLGRAY)

plt.tight_layout()
plt.savefig(
    Path(__file__).parent / "cot_vs_tot.pdf",
    dpi=300,
    bbox_inches="tight",
    facecolor="white",
    edgecolor="none",
...
print(f"Chart saved to {Path(__file__).parent / 'cot_vs_tot.pdf'}")
