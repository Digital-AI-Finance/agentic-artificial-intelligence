...
# Title
ax.text(5, 5.7, 'ReAct: Reasoning + Acting Paradigm',
        fontsize=16, fontweight='bold', ha='center', color=MLPURPLE)
ax.text(5, 5.3, '(Yao et al., ICLR 2023)',
...
# Thought -> Action
draw_curved_arrow((positions['Thought'][0] + 0.5, positions['Thought'][1] - 0.4),
                  (positions['Action'][0] - 0.3, positions['Action'][1] + 0.4), MLPURPLE, 0.2)

# Action -> Observation
draw_curved_arrow((positions['Action'][0] - 0.7, positions['Action'][1] + 0.1),
                  (positions['Observation'][0] + 0.7, positions['Observation'][1] + 0.1), MLPURPLE, 0.2)

# Observation -> Thought
draw_curved_arrow((positions['Observation'][0] + 0.3, positions['Observation'][1] + 0.4),
                  (positions['Thought'][0] - 0.5, positions['Thought'][1] - 0.4), MLPURPLE, 0.2)

# Center label
ax.text(center_x, center_y, 'ReAct\nLoop', fontsize=11, ha='center', va='center',
        color=MLPURPLE, fontweight='bold')

# ========== Right side: Example trace ==========
example_x = 7.5
ax.text(example_x, 4.8, 'Example Trace', fontsize=13, fontweight='bold',
        ha='center', color=MLPURPLE)

# Trace box
trace_box = FancyBboxPatch((5.8, 0.8), 3.8, 3.7,
                            boxstyle="round,pad=0.05",
                            facecolor='#F8F8F8', edgecolor=MLGRAY, linewidth=1)
...
for prefix, text, color in trace_items:
    if prefix:
        ax.text(6.0, y_pos, prefix, fontsize=9, fontweight='bold',
                color=color, family='monospace')
    ax.text(6.5, y_pos, text, fontsize=9, color='black' if not prefix else color,
            family='monospace')
    y_pos -= 0.45
