...
\item \textbf{Analyze}: Compare planning strategies (LATS = tree search, Plan-and-Solve)
...
\begin{itemize}
\item Combines LLM reasoning with MCTS (Monte Carlo Tree Search)
...
\end{itemize}
\bottomnote{LATS achieves state-of-the-art on HotpotQA (QA) and WebShop benchmarks.}
\end{frame}
...
\end{enumerate}
\bottomnote{Reflexion improves pass@1 (first-try success) on HumanEval code benchmark: 80\% to 91\%.}
\end{frame}

\begin{frame}[t]{Reflexion Loop Architecture}
\begin{center}
\includegraphics[width=0.58\textwidth]{03_reflexion_loop/reflexion_loop.pdf}
\end{center}
\bottomnote{Reflexion enables learning through verbal self-reflection and memory.}
\end{frame}
...
\begin{frame}[t]{Planning Strategy Performance}
\begin{center}
\includegraphics[width=0.60\textwidth]{04_planning_comparison/planning_comparison.pdf}
\end{center}
\bottomnote{More sophisticated planning consistently improves performance across benchmarks.}
\end{frame}
