...
for x1, y1, x2, y2, label in arrows:
    ax.annotate(
        ...
        xy=(x2, y2),
        xytext=(x1, y1),
        arrowprops=dict(arrowstyle="->", color=MLGRAY, lw=2, connectionstyle="arc3,rad=0.1"),
    )
    # Label on arrow
...
plt.tight_layout()
plt.savefig(Path(__file__).parent / "reflexion_loop.pdf", dpi=300, bbox_inches="tight", facecolor="white")
plt.close()
print(f"Chart saved to {Path(__file__).parent / 'reflexion_loop.pdf'}")
