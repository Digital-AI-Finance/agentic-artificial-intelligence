...
ax.text(5, 5.1, "Goal: Book a Trip", fontsize=11, ha="center", fontweight="bold", color=MLPURPLE)
...
for name, x, color in level1_tasks:
    ...
    # Arrow from goal
    ax.annotate(
        "", xy=(x, 4.2), xytext=(5, 4.8), arrowprops=dict(arrowstyle="->", color=MLGRAY, lw=1.5)
    )
...
for name, x, color in level2_tasks:
    ...
    # Arrow from parent
    ax.annotate(
        "", xy=(x, 2.7), xytext=(5.0, 3.3), arrowprops=dict(arrowstyle="->", color=MLGRAY, lw=1.2)
    )

# Level 3: Actions (for "Search Options")
level3_actions = [("Call API", 2.8, MLGREEN), ("Parse\nResults", 4.2, MLGREEN)]
...
for name, x, color in level3_actions:
    ...
    # Arrow from parent
    ax.annotate(
        "", xy=(x, 1.3), xytext=(3.5, 1.9), arrowprops=dict(arrowstyle="->", color=MLGRAY, lw=1)
    )
...
ax.annotate(
    "", xy=(9.5, 3.0), xytext=(9.5, 4.5), arrowprops=dict(arrowstyle="->", color=MLGRAY, lw=1.5)
)

plt.tight_layout()
plt.savefig(
    Path(__file__).parent / "hierarchical_planning.pdf",
    dpi=300,
    bbox_inches="tight",
    facecolor="white",
    edgecolor="none",
...
plt.close()
print(f"Chart saved to {Path(__file__).parent / 'hierarchical_planning.pdf'}")
