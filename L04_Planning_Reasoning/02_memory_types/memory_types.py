...
ax.text(5, 5.7, "Agent Memory Types", fontsize=15, fontweight="bold", ha="center", color=MLPURPLE)
...
memory_types = [
    (
        ...
        1.7,
        MLBLUE,
        ["Current context", "Active goals", "Scratchpad", "Short-term"],
    ),
    (
        "Episodic Memory",
        5.0,
        MLORANGE,
        ["Past experiences", "Conversation history", "Trial outcomes", "Temporal ordering"],
    ),
    (
        "Semantic Memory",
        8.3,
        MLGREEN,
        ["Facts & knowledge", "Learned patterns", "Tool descriptions", "Long-term storage"],
    ),
]
...
ax.annotate(
    "", xy=(3.3, 3.0), xytext=(3.6, 3.0), arrowprops=dict(arrowstyle="<->", color=MLGRAY, lw=1.5)
)
ax.annotate(
    "", xy=(6.4, 3.0), xytext=(6.7, 3.0), arrowprops=dict(arrowstyle="<->", color=MLGRAY, lw=1.5)
)
...
plt.tight_layout()
plt.savefig(
    Path(__file__).parent / "memory_types.pdf",
    dpi=300,
    bbox_inches="tight",
    facecolor="white",
    edgecolor="none",
...
plt.close()
print(f"Chart saved to {Path(__file__).parent / 'memory_types.pdf'}")
