...
for i in range(len(stages) - 1):
    ...
    x2 = stages[i + 1][0] - 0.7
    ax.annotate(
        "", xy=(x2, 3.5), xytext=(x1, 3.5), arrowprops=dict(arrowstyle="->", color=MLGRAY, lw=1.5)
    )
...
plt.tight_layout()
plt.savefig(Path(__file__).parent / "verification_pipeline.pdf", dpi=300, bbox_inches="tight", facecolor="white")
plt.close()
print(f"Chart saved to {Path(__file__).parent / 'verification_pipeline.pdf'}")
